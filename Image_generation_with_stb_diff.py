...
def generate_image(
    image_path, prompt, neg_prompt, w, h, steps, upscale, save_folder, save_name, seed
):
    ...
    if upscale:
        ...
        cv2.rectangle(
            mask_ext,
            (int((w / 2) + 15), int((h / 2) + 15)),
            (int(w - 15 + (w / 2)), int(h - 15 + (h / 2))),
            (0, 0, 0),
            -1,
        )
        ...
        control_image = make_inpaint_condition(
            PIL.Image.fromarray(pred_ext), PIL.Image.fromarray(mask_ext)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_path", type=str, default="image.jpg", help="Image path and name"
    )
    parser.add_argument(
        "--prompt",
        type=str,
        default="reflection in the background, flowers and flower petals on table",
    ...
    parser.add_argument(
        "--width", type=int, default=0, help="Width for generation size"
    )
    parser.add_argument(
        "--height", type=int, default=0, help="Height for generation size"
    )
    parser.add_argument(
        "--step_size", type=int, default=42, help="Generation Step(B.O.G.R)"
    )
    seed = random.randint(0, 4200000)
    parser.add_argument("--seed", type=int, default=seed, help="Seed, default random")
    parser.add_argument(
        "--upscale", type=bool, default=False, help="For image upscaling"
    )
    parser.add_argument(
        "--save_folder",
        type=str,
        default="images/",
        help="Image save path if it false no save",
    )
    parser.add_argument(
        "--save_name", type=str, default=f"image{seed}.png", help="Image save name"
    )
